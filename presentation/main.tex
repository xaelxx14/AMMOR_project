%----------------------------------------------------------------------------------------
%    PACKAGES AND THEMES
%----------------------------------------------------------------------------------------

\documentclass[aspectratio=169,xcolor=dvipsnames]{beamer}
\usepackage{caption}
\usepackage{biblatex}
\addbibresource{references.bib} 
\usepackage{hyperref}
\usepackage{graphicx} % Allows including images
\usepackage{booktabs} % Allows the use of \toprule, \midrule and \bottomrule in tables
\setbeamertemplate{footline}[frame number] % Adds slide numbers to the footer
\usepackage{amsmath} % Allows the use of mathematical symbols and environments
\usepackage{amsthm} % Allows the use of theorem environments
\usepackage{amssymb} % Allows the use of additional mathematical symbols
\usepackage{multicol} % Allows the use of multiple columns in a slide
\usepackage{algorithm} % For algorithm environment
\usepackage{algpseudocode} % For algorithmic environment

\usepackage[progressbar=yes,framecount=fancy]{BeamerTheme}
%----------------------------------------------------------------------------------------
%    TITLE PAGE
%----------------------------------------------------------------------------------------

\title{On the Exact Separation of \\ Mixed Integer Knapsack Cuts}

\author{
Guillem Audebert, 
Léonard Bultinck,
Tudor Cardas,
Yağmur Ergin}
\date{\today} % Date, can be changed to a custom date

\institute{
    \vspace{0.5cm}
    \small Based on the paper \textit{On the Exact Separation of Mixed Integer Knapsack Cuts} \cite{cuts2007exact} by \textbf{Ricardo Fukasawa, Marcos Goycoolea}.

}
%----------------------------------------------------------------------------------------
%    PRESENTATION SLIDES
%----------------------------------------------------------------------------------------

\begin{document}

\begin{frame}{Overview}
    % Automatically generates a table of contents based on \section{} and \subsection{}
    \begin{block}{Task Splitting}
    	\begin{itemize}
    		\item Yağmur Ergin : Introduction and construction of the problem 
    		\item Guillem Audebert : Slimpification and precomputation steps
    		\item Tudor Cardas : Optimization steps
    		\item Léonard Bultinck : Numercial results and interpretation
    	\end{itemize}
    \end{block}
    \vspace{0.5cm}
\end{frame}

%------------------------------------------------
\section{Introduction}
%------------------------------------------------

\begin{frame}{Introduction}
    \vspace{-0.2cm}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{Mixed Integer Knapsack Set :} \\
            \small
            Consider a positive integer $n$, $b \in \mathbb{Q}$, $a \in \mathbb{Q}^n$, $l \in \{\mathbb{Q} \cup \{-\infty\}\}^n$, $u \in \{\mathbb{Q} \cup \{+\infty\}\}^n$ and $I \subseteq N := \{1, \ldots, n\}$
            \[
            K = \{x \in \mathbb{R}^n : ax \leq b, l \leq x \leq u, x_i \in \mathbb{Z}, \forall i \in I \}
            \]
            Given a point $x^* \in \mathbb{Q}^n$
            \begin{itemize}
                \item Identify an inequality ($\pi x \leq \pi_0$) valid for conv$(K)$ and violated by $x^*$.
                \item Prove that no such inequality exists.
            \end{itemize}
        \end{column}
        \hspace{0.5cm}
        \begin{column}{0.48\textwidth}
            \textbf{Mixed Integer Set : } \\
            \vspace{-0.4cm}
            \small
            \[
            P = \{x \in \mathbb{R}^n : Dx \leq d, l \leq x \leq u, x_i \in \mathbb{Z}, \forall i \in I \}
            \]
            $D$ is an $m \times n$ matrix, and $d \in \mathbb{Q}^n$\\
            \vspace{0.3cm}
            If $(a, b)$ can be obtained as a non-negative linear combination of rows from $(D, d)$, then $P \subseteq K$ 
            and we say that $K$ is an implied knapsack set of $P$. 
        \end{column}
    \end{columns}
\end{frame}


\begin{frame}{Is $x^* \in \text{conv}(K)$?}
	\vspace{-0.4cm}
    \textbf{Suggestion:} Solve the following linear program to address the separation problem:\\
    Let $\{x_1, x_2, \ldots, x_q\}$ and $\{r_1, r_2, \ldots, r_t\}$ represent the extreme points and rays of $\text{conv}(K)$.
	\vspace{-0.4cm}
    \small
	\begin{align*}
		LP_1 : \min \quad & \sum_{i=1}^{n} (u_i + v_i) \\
		\text{s.t.} \quad & \pi x^k - \pi_0 \leq 0 & \forall k = 1, \ldots, q & \quad \text{(C1)} \\
		& \pi r^k \leq 0 & \forall k = 1, \ldots, t & \quad \text{(C2)} \\
		& \pi x^* - \pi_0 = 1 & & \quad \text{(C3)} \\
		& \pi + u - v = 0 & & \quad \text{(C4)} \\
		& u \geq 0, \; v \geq 0 & &
	\end{align*}
	\vspace{-0.1cm}
	If $LP_1$ is infeasible, then $x^* \in \text{conv}(K)$.\\ 
	Otherwise, the optimal solution $(u, v, \pi, \pi_0)$ gives a valid knapsack cut $\pi x \leq \pi_0$ \cite{applegate2001tsp}.
\end{frame}

\begin{frame}{Algorithm}
	\begin{algorithm}[H]
		\caption{knapsack\_separator($K$, $x^*$)}
		\small
		\textbf{Input:} A MIK set $K$, and a vector $x^*$.\\
		\textbf{Output:} If $x^* \in \text{conv}(K)$ : TRUE; Else : FALSE and a cut separating $x^*$ from $\text{conv}(K)$\\
		\vspace{-0.4cm}
		\begin{algorithmic}[1]
			\State Simplify the problem by fixing variables which are at their bounds, and reduce to a smaller dimensional knapsack separation problem;
			\State Formulate problem $LP_1$ in the reduced variable space without $(C1) - (C2)$;
			\State Apply P1-P2 to simplify $LP_1$;
			\State Solve $LP_1$ using the dynamic row generation algorithm;
			\State If solving $LP_1$ reveals that $x^* \in \text{conv}(K)$ : STOP and return TRUE;
			\State Let $(\hat{\pi}, \hat{\pi}_0)$ represent the optimal solution to $LP_1$;
			\State Lift cut $\hat{\pi} x \leq \hat{\pi}_0$ to obtain a cut $\pi x \leq \pi_0$ which is valid for $K$;
			\State STOP and return the cut $(\pi, \pi_0)$;
		\end{algorithmic}
	\end{algorithm}
\end{frame}

%------------------------------------------------
\section{Reducing $LP_1$}
%------------------------------------------------
%%%%%%%%%%%%%%% Guillem

\begin{frame}{Is $x^* \in \text{conv}(K)$ ?}

	\begin{block}{$LP_1$ and knapsack cut}
		\begin{itemize}
			\item If not, exhibit a cut $\pi x \leq \pi_0$ 
			\item Constraint $ \pi x^k - \pi_0 \leq 0 \; , \forall x^k$ being an extreme point $\text{conv}(K)$.
			\item $\implies $ exponential number of rows.
			\item Too many $\pi$ variables.
		\end{itemize}
	\end{block}
	
	\begin{center}
		$\Rightarrow$ \textbf{Simplify $LP_1$ before solving it}
	\end{center}
\end{frame}

\begin{frame}{Reducing Dimension: Fixing \& Lifting}
	
	\begin{columns}
		\begin{column}{0.48\textwidth}
			\begin{block}{Step 1: FIXING}
				\begin{itemize}
					\item Identify $L = \{i : x^*_i = l_i\}$ \\
					and $U = \{i : x^*_i = u_i\}$
					\item Fix these variables
					\item Solve on $K(L,U)$ \\
					\textcolor{blue}{(smaller space)}
				\end{itemize}
			\end{block}
		\end{column}
		
		\begin{column}{0.48\textwidth}
			\begin{block}{Step 2: LIFTING}
				\begin{itemize}
					\item Cut found: $\pi x \leq \pi_0$
					\item Reintroduce fixed variables one by one
					\item Obtain valid cut for full $K$
				\end{itemize}
			\end{block}
		\end{column}
	\end{columns}
	
	\vspace{0.4cm}
	\begin{exampleblock}{Key observation}
		$x^* \in \text{conv}(K) \Leftrightarrow x^* \in \text{conv}(K(L,U))$
	\end{exampleblock}
	
\end{frame}

\begin{frame}{Eliminate variables \& Tighten $LP_1$}

	\begin{block}{Proposition 1 : Using property of valid cuts}
		\begin{itemize}
			\item Sign coherence: $a_i > 0 \Rightarrow \pi_i \geq 0$ \quad ; \quad $a_i < 0 \Rightarrow \pi_i \leq 0$
			\item Unbounded continuous variables $\Rightarrow$ \textbf{eliminate} or replace by $\alpha \cdot a_i$
		\end{itemize}
	\end{block}


\end{frame}



\begin{frame}{Eliminate variables \& Tighten $LP_1$}
	\begin{block}{ Proposition 2 : Using position of $x^*$}
		
			Variables at bounds in "right direction" $\Rightarrow$ set $\pi_i = 0$
			
			\begin{definition}
				\begin{itemize}
					\item $H^+ = \{i : a_i > 0$ and $x^*_i = l_i\}$ 
					\item $H^- = \{i : a_i < 0$ and $x^*_i = u_i\}$ 
				\end{itemize}
				$\Rightarrow$ Set $\pi_i = 0$ for all $i \in H^+ \cup H^-$
			\end{definition}
			
	\end{block}
	
		\vspace{0.3cm}
	\begin{exampleblock}{Combined effect}
		Transform unsolvable $LP_1$ into  problem that solvers can handle
	\end{exampleblock}
\end{frame}





%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Tudor
%------------------------------------------------
\section{Solving the Mixed Integer Knapsack Problem}

\begin{frame}{Detecting Unbounded MIKP}
    We're interested now in solving the problem $\max \{ cx : x \in K \} $
    \begin{block}{Conditions for Unboundedness}
        An MIKP instance is unbounded if and only if one of the following holds (checkable in $O(n)$ time):
        \begin{itemize}
            \item \textbf{Condition A (Single Variable):} There exists an index $i$ such that:
            \[ a_i \leq 0, \quad c_i > 0, \quad \text{and} \quad u_i = +\infty \]
            
            \item \textbf{Condition B (Pairwise):} There exist indices $i, j$ such that $e_i > e_j$ and:
            \[
            (a_i > 0, c_i > 0, u_i = +\infty) \quad \text{and} \quad (a_j < 0, c_j \leq 0, u_j = +\infty)
            \]
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}{Preprocessing: Definitions}
    \begin{center}
        Let $N^+ = \{i : a_i > 0\}$ and $N^- = \{i : a_i < 0\}$.
        
        \vspace{0.3cm}
        
        \Large
        \[
        L_{min} = \sum_{i \in N^-} a_i u_i + \sum_{i \in N^+} a_i l_i
        \]
        
        \vspace{0.1cm}
        
        \[
        L_{max} = \sum_{i \in N^-} a_i l_i + \sum_{i \in N^+} a_i u_i
        \]
    \end{center}
\end{frame}

\begin{frame}{Preprocessing: 8 Steps}
    \small
    \setlength{\itemsep}{0.5pt} % Reduce vertical space between items
    \vspace{-0.5cm}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \begin{enumerate}
                \item \textbf{Detect Infeasibility:} \\
                If $L_{min} > b$ or if any $l_i > u_i$.
                
                \item \textbf{Detect Unboundedness:} \\
                Apply the $O(n)$ check from previous slide.
                
                \item \textbf{Fix Variables:} \\
                Fix $x_i = u_i$ if $c_i \geq 0, a_i \leq 0$. \\
                Fix $x_i = l_i$ if $c_i \leq 0, a_i \geq 0$.
                
                \item \textbf{Tighten Upper Bounds:} \\
                Update $u_i$ using knapsack capacity and $L_{min}$.
            \end{enumerate}
        \end{column}
        \hspace{0.2cm} % Reduced horizontal space
        \begin{column}{0.48\textwidth}
            \begin{enumerate}
                \setcounter{enumi}{4}
                \item \textbf{Tighten Lower Bounds:} \\
                Update $l_i$ using knapsack capacity and $L_{min}$.
                
                \item \textbf{Sort Variables:} \\
                Sort by decreasing efficiency $e_i = c_i/a_i$. \\
                (Break ties: Integers before Continuous).
                
                \item \textbf{Aggregate Integers:} \\
                Merge integer variables with identical $a_i, c_i$.
                
                \item \textbf{Aggregate Continuous:} \\
                Merge continuous variables with same efficiency $e_i$ and sign of $a_i$.
            \end{enumerate}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Domination: Definitions}
    \begin{block}{Cost-Domination}
        $x^1$ \textbf{cost-dominates} $x^2$ if:
        \[ cx^1 > cx^2 \quad \text{and} \quad ax^1 \leq ax^2 \]
    \end{block}

    \vspace{0.5cm}

    \begin{block}{Lexicographic-Domination}
        $x^1$ \textbf{lexicographically-dominates} $x^2$ if $cx^1 = cx^2$, $ax^1 < ax^2$, and $x^1$ is lexicographically smaller than $x^2$ (at the first different index $i$, $x^1_i < x^2_i$).
    \end{block}
\end{frame}

\begin{frame}{Domination: Integer Variables (Tuples)}
    \small
    \begin{block}{Domination Tuple $(i, j, k_i, k_j)$}
        Integers $k_i, k_j$ form a tuple if swapping $k_j$ items of $j$ for $k_i$ items of $i$ improves the solution:
        \[ a_i k_i + a_j k_j \geq 0 \quad \text{and} \quad c_i k_i + c_j k_j < 0 \]
    \end{block}

    \vspace{0.1cm}

    \begin{block}{Theorem 1 (Pruning Rule)}
        If such a tuple exists and in the current node $l_i + k_i \leq x_i \leq u_i + k_i$, we can impose a bound on $x_j$ without losing optimal solutions:
        \begin{itemize}
            \item If $k_j > 0$: Impose $x_j \leq l_j + k_j - 1$
            \item If $k_j < 0$: Impose $x_j \geq u_j + k_j + 1$
        \end{itemize}
    \end{block}
\end{frame}

\begin{frame}{Domination: Mixed Variables}
    \small
    \begin{block}{Proposition 7 \& 8 (Intuition)}
        If we have a more efficient integer variable $x_i$ available ($e_k < e_i$), we should prioritize it over a less efficient continuous variable $x_k$.
    \end{block}
    
    \vspace{0.3cm}
    
    \textbf{Branching Implication:}
    If $x_i \leq u_i - 1$ (integer variable not full) and $e_k < e_i$, we can fix continuous variables $j$ that are "worse" than $k$:
    \[ x_j = l_j \quad \forall j \in C_{f+1,n} \]
    This forces the solver to use the efficient integer item $x_i$ before filling the knapsack with inefficient continuous items.
\end{frame}

%------------------------------------------------
%Leonard
\section{Computational Results}

\begin{frame}{Experimental Setup}
The computational results are based on \textbf{67 instances} of the knapsack problem.

\begin{itemize}
    \item These instances are selected from an initial set of approximately \textbf{130,000 instances}.
    \item Most of the instances are similar; therefore, only the \textbf{latest (supposedly hardest)} instance of each group is kept.
    \item The \textbf{easiest instances} (solved in less than 1 second) are removed.
\end{itemize}

\medskip
Removing very easy instances reduces sensitivity to small time variations and avoids bias in performance comparisons.
\end{frame}

\begin{frame}{Limitations of the Instance Selection}
Despite the above filtering, using only \textbf{67 instances} may appear insufficient for a robust comparison.

\begin{itemize}
    \item The original 130,000 instances are drawn from \textbf{MIPLIB}.
    \item The selection process is not fully described, making the results \textbf{non-reproducible}.
    \item Fortunately, the authors make the full set of instances \textbf{available upon request}.
\end{itemize}
\end{frame}

\begin{frame}{Analysis of the Performance Graph}
The performance graph is difficult to read but provides valuable information.

\begin{itemize}
    \item It shows which solver performs best on a given \textbf{percentage of instances}.
    \item This provides a notion of \textbf{average dominance or “betterness”}.
    \item The curve also indicates the percentage of instances for which a solver is more than $x$ times slower than another.
\end{itemize}

Overall, it is an effective way to compare solvers across multiple instances.
\end{frame}

\begin{frame}{Discussion and Limitations}
While this approach is useful, it has some limitations.

\begin{itemize}
    \item As more solvers are added, the graph becomes harder to interpret.
    \item The use of an \textbf{OR condition on the y-axis} leads to information loss.
\end{itemize}

Thus, although informative, the visualization does not scale well with an increasing number of solvers.
\end{frame}

\begin{frame}{Results}
\begin{columns}[T]
    \begin{column}{0.80\textwidth}
        \begin{figure}
            \vspace{-0.7cm}

            \centering
            \includegraphics[width=0.70\textwidth]{figure1}
            \caption{Performance comparison of solvers on knapsack instances}
        \end{figure}
    \end{column}
    \begin{column}{0.20\textwidth}
    \end{column}
\end{columns}
\end{frame}

\begin{frame}{Comparative Analysis}
The comparison highlights the strong performance of the proposed algorithm, \textbf{KBB}.

\begin{itemize}
    \item KBB improves rapidly along the performance graph $\implies$ no instances inherently hard for the algorithm
    \item It is \textbf{never more than 1000 times slower} than any competing solver on any instance.
    \item KBB performs best on approximately \textbf{77\% of the instances}.
    \item There are \textbf{two instances} that could not be solved by either \textbf{CPX} or \textbf{CPP}, while \textbf{KBB} successfully solved them.
\end{itemize}

Overall, these results indicate both robustness and competitive efficiency of KBB.
\end{frame}

\begin{frame}{Enhancement results}
\begin{columns}[T]
    \begin{column}{0.80\textwidth}
        \begin{figure}
            \centering
            \vspace{-0.7cm}
            \includegraphics[width=0.70\textwidth]{figure2}
            \caption{Performance comparison of enhancement induced by domination branching or reduced-cost bound improvement}
        \end{figure}
    \end{column}
    \begin{column}{0.20\textwidth}
    \end{column}
\end{columns}
\
\end{frame}

\begin{frame}{Enhancement Analysis}


\begin{itemize}
    \item When applied independently, the enhancements do not reduce the search space and may even slow down the algorithm.
    \item When combined, however, they lead to a \textbf{significant performance improvement}.
    \item These two features naturally complement each other.
    \item Updating bounds enables additional domination conditions to be applied.
    \item This interaction results in \textbf{further pruning of the search tree}.
\end{itemize}
\end{frame}
%------------------------------------------------

\begin{frame}{References}
    \tiny
    \setbeamertemplate{bibliography item}{} % Remove the icon/emoji
    \begin{multicols}{2} 
        \printbibliography[heading=none]
    \end{multicols}
\end{frame}
%----------------------------------------------------------------------------------------

\end{document}
